...
class person
{
  private:
    ...
    int d_phoneNumber;
  public:
    person(const char *name = "",const char *address = "",const int phoneNumber = 0); // constructor
    ~person(void);                                          // destructor
    void set_name(const char *name);
    void set_address(const char *address);
    void set_phoneNumber(const int phoneNumber);
...
void person::set_phoneNumber(const int phoneNumber)
{
  d_phoneNumber = phoneNumber;
}

person::person(const char *name,const char *address, const int phoneNumber)
{ // constructor
  if(debug != 0)
...
int main(void)
{
  ...
  person m("Maria","no address",1234567);
  cout << m;
  return 0;
}
